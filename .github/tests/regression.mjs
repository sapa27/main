...
const REV = 'r331';
const RPC_REV = 'r330';
const RELEASE = 'commission-v1.2-p2f-final-consolidated-2026-09-05-r331-v62';
const ASSET = 'asset-manifest-r331-v62-p2f';
...
const P2F_FINGERPRINT_RE=/p2f-sha256-[0-9a-f]{64}/g;
...
ok('R331/v62 frontend release converges across public Pages files',()=>{
  for(const [name,text] of [['index',index],['runtime',runtime],['config',config],['transport',transport],['workflow',workflow]])assert.ok(text.toLowerCase().includes(REV),`missing ${REV} in ${name}`);
...
ok('frontend revision is separated from the stable RPC protocol',()=>{
  const stale=[];
  for(const [name,text] of [['index',index],['runtime',runtime],['config',config],['transport',transport],['workflow',workflow]]){
...
ok('GAS endpoint is canonical /exec URL',()=>{
  ...
  assert.ok(index.includes('./app-critical-runtime.js?v=r331-v62-p2f'));
  assert.ok(index.includes('./github-gas-transport.js?v=r331-v62-p2f'))
});
...
ok('workflow separates deterministic regression from live GAS release attestation',()=>{
  assert.ok(workflow.includes('Run R331/v62 automated regression suite (static, contract-aware)'));
  assert.ok(workflow.includes('gas_attestation:'));
  assert.ok(workflow.includes('P2-F attest live GAS release before Pages deploy'));
...
ok('backend router handler permission and write-CSRF contracts have static parity',()=>{
  ...
  const allowedUnrouted=new Set(['apiRouter','apiGetPhase0ContractGate','apiGetPhase1Contract','apiGetPhase2Contract']);
...
ok('all write routes converge on the canonical root write gateway and recovery ledger',()=>{
  ...
  console.log(`# P1-B gateway audit: writes=${writes.length}, canonicalGateway=${writes.length}, rootFlush=1, failedMutationRecovery=2`);
});
...
ok('write invalidation contracts close over backend cache domains and frontend refresh dependencies',()=>{
  ...
  console.log(`# P1-B cache/refresh audit: writeContracts=${writeMethods.length}, invalidationProfiles=closed, frontendDependencyGaps=0`);
});
...
ok('P1-C read paths honor canonical fresh mode and generation-stamped dependencies',()=>{
  ...
  console.log('# P1-C read-cache audit: Dashboard/Search/Meeting/Tracking/People/Budget fresh-mode parity=closed, source-stamp gaps=0');
});

ok('P1-D async reads enforce latest-request-wins and reject transport-empty responses',()=>{
  if(!FULL)return;
  ...
  console.log('# P1-D async audit: critical lanes=Dashboard/Search/Report/Meeting/Petitioner/People/Budget, stale UI overwrite gaps=0, transport-empty admission=guarded');
});

ok('P1-E session/auth and route transitions suppress obsolete request contexts',()=>{
  if(!FULL)return;
  const lifecycle=file('gas-backend/Runtime_01_Request_Lifecycle.html'),
  ...
  ])assert.ok(lifecycle.includes(token),`P1-E lifecycle context guard missing: ${token}`);
  ...
  console.log('# P1-E context audit: auth epoch=login/resume/logout, route epoch=page-changing, in-flight reuse reset=guarded, activation generation=guarded');
});

ok('P2 end-to-end production reliability / performance gate is release-blocking and non-destructive',()=>{
  const measurement=scriptById(index,'app-production-measurement-gate-current');
  if(FULL){
    const gasIndex=file('gas-backend/Index.html');
    assert.equal(measurement,scriptById(gasIndex,'app-production-measurement-gate-current'),'P2 measurement owner must remain exact dual-host parity');
  }
  for(const token of [
  ...
  ])assert.ok(measurement.includes(token),`P2 client production gate missing: ${token}`);
  assert.ok(measurement.includes('method==="apiSaveCase"||method==="apiSaveMeetingLog"||method==="apiSaveLetter"'),'P2 critical journey must observe a canonical production save without generating one');
  assert.ok(measurement.includes('CRITICAL_E2E_STEPS[criticalStep]==="refresh"&&isFreshInput(input)'),'P2 save must be followed by a verified fresh read');
  assert.ok(!measurement.includes('password:'),'P2 evidence must never embed credentials');
  if(FULL){
    ...
    ])assert.ok(core.includes(token),`P2 server release gate missing: ${token}`);
    for(const token of ['endToEndProductionReliabilityGate: !0','criticalJourneySequenceAudit: !0','journeyTimeoutAdmissionAudit: !0','postDeploySmokeGate: !0','p2-production-e2e-reliability-performance-contract'])
      assert.ok(quality.includes(token),`P2 quality contract missing: ${token}`);
  }
  for(const token of ['production_smoke:','needs: deploy','page_url: ${{ steps.deployment.outputs.page_url }}','P2-F post-deploy dual-host release / performance smoke','pageMaxMs>8000','health.ms>20000','rpcRoundTripMs>45000'])
    assert.ok(workflow.includes(token),`P2 post-deploy CI smoke missing: ${token}`);
  console.log('# P2 production gate: critical flow=9 ordered steps, observed writes only, fresh-after-write required, resume/logout timed, post-deploy Pages+GAS smoke enabled');
});

ok('P2-F immutable release fingerprint and dual-deploy attestation remain source-derived',()=>{
  const configured=configuredSourceFingerprint(config);
  assert.match(configured,/^p2f-sha256-[0-9a-f]{64}$/,'frontend source fingerprint format');
  assert.ok(index.includes('P2-F IMMUTABLE RELEASE FINGERPRINT '+configured+'; DUAL DEPLOY ATTESTATION REQUIRED'));
  assert.ok(config.includes('sourceFingerprint:SOURCE_FINGERPRINT'));
  for(const token of ['gas_attestation:','needs: [regression, gas_attestation]','health.releaseStamp !== expectedRelease','health.sourceFingerprint !== expectedFingerprint','resultPayload.releaseStamp!==expectedRelease||resultPayload.sourceFingerprint!==expectedFingerprint','deployedFingerprint!==expectedFingerprint'])assert.ok(workflow.includes(token),`P2-F workflow attestation missing: ${token}`);
  if(FULL){
    ...
    assert.equal(configured,derived,'committed P2-F fingerprint must equal deterministic production source SHA-256');
    assert.equal(coreFingerprint,derived,'GAS release fingerprint must equal frontend release fingerprint');
    assert.ok(gasIndex.includes('P2-F IMMUTABLE RELEASE FINGERPRINT '+derived+'; DUAL DEPLOY ATTESTATION REQUIRED'));
    for(const token of ['releaseStamp: APP_DEPLOY_RELEASE.stamp','sourceFingerprint: APP_DEPLOY_RELEASE.sourceFingerprint','immutableReleaseFingerprintAudit: !0','dualDeployAttestationAudit: !0','liveGasReleaseMatchRequiredBeforePagesDeploy: !0','productionSmokeReleaseMatchRequired: !0'])assert.ok(core.includes(token)||quality.includes(token),`P2-F backend attestation contract missing: ${token}`);
  }
  console.log('# P2-F release fingerprint: '+configured);
});

ok('dual-host release and reliability owners stay aligned',()=>{
  if(!FULL)return;
  const gasIndex=file('gas-backend/Index.html');
  assert.ok(index.includes('DUAL-HOST CONTRACT p0d-r331-v62.2'));
  assert.ok(gasIndex.includes('CANONICAL GAS FRONTEND r331-v62; GAS RPC PROTOCOL r330; DUAL-HOST CONTRACT p0d-r331-v62.2'));
...
ok('P2-C client permission fallbacks and page aliases delegate to the canonical permission matrix',()=>{
  ...
  assert.equal(frontendMatrix.stamp,'permission-matrix-p2c-2026-09-05-r330');
  ...
  if(FULL){
    ...
    assert.ok(permission.includes('stamp: "permission-matrix-p2c-2026-09-05-r330"'),'backend permission matrix stamp drift');
    ...
    for(const token of ['canonicalPermissionFallbackAudit: !0','clientPageAliasCanonicalAudit: !0','permissionMatrixDualHostParityAudit: !0','legacyPermissionBranchCount: 0','critical-runtime-canonical-delegation'])assert.ok(quality.includes(token),`P2-C quality contract missing: ${token}`);
  }
});

ok('P2-F CSP/PWA and dead-metadata/navigation consolidation stay canonical',()=>{
  const cspToken='data-app-csp="p2f-compatible-baseline"';
  ...
  if(FULL){
    ...
    for(const token of ['cspCompatibilityAudit: !0','pwaManifestMimeAudit: !0','deadBootstrapMetadataPrunedAudit: !0','deadAssetManifestMetadataPrunedAudit: !0','canonicalNavigationAliasAudit: !0','redundantLogoAliasCount: 0'])assert.ok(quality.includes(token),`P2-F quality contract missing: ${token}`);
  }
});

ok('artifact performance budgets',()=>{
  assert.ok(Buffer.byteLength(index,'utf8')<=460000,'index.html exceeds P2-F 460 KB budget');
  ...
  assert.ok(Math.max(...blocks.map(x=>Buffer.byteLength(x,'utf8')))<=60000,'single inline script exceeds P2-F 60 KB budget');
});

ok('P2-F preserves the P2-B external runtime order and file count',()=>{
  assert.ok(index.includes('<script id="app-release-config-current">'),'release config must execute before transport');
  const configPos=index.indexOf('id="app-release-config-current"'), transportPos=index.indexOf('src="./github-gas-transport.js?v=r331-v62-p2f"'), foundationPos=index.indexOf('id="app-critical-foundation-consumer"'), runtimePos=index.indexOf('src="./app-critical-runtime.js?v=r331-v62-p2f"'), logoutPos=index.indexOf('id="login-after-logout-no-refresh-fix-currentStamp"');
  ...
  if(FULL){const quality=file('gas-backend/Code_06_Platform_QualityGates.gs');for(const token of ['githubCriticalRuntimeExternalizedAudit: !0','githubIndexGodHtmlReductionAudit: !0','externalRuntimeExecutionOrderPreservedAudit: !0'])assert.ok(quality.includes(token),`P2-F inherited external-runtime quality contract missing: ${token}`)}
});
