# GitHub Pages + Google Apps Script — R132

ชุด R132 ใช้ GAS Web App deployment ต่อไปนี้เป็น backend:
...
ไฟล์ JavaScript local ทั้งหมดต้องโหลดด้วย `?v=r132`
...
3. ตรวจ Network ให้ JavaScript local ทั้ง 7 ไฟล์เป็น `r132`
...
## R132 — Authenticated data loading stability
...
- The persistent verified bridge remains an automatic fallback when available.
...
The backend must be deployed first because the R132 frontend requires the R132 nonce to be echoed in `GAS_API_POST_RESPONSE`.
