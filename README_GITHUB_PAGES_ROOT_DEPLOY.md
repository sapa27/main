# GitHub Pages + Google Apps Script — R140

ชุด R140 ใช้ GAS Web App deployment ต่อไปนี้เป็น backend:
...
ไฟล์ JavaScript local ทั้งหมดต้องโหลดด้วย `?v=r140`
...
3. ตรวจ Network ให้ JavaScript local ทั้ง 7 ไฟล์เป็น `r140`
...
## R140 — Authenticated data loading stability
...
The backend must be deployed first because the R140 frontend requires the R140 nonce to be echoed in `GAS_API_POST_RESPONSE`.


## R140 — GitHub Bridge-primary / POST fallback bridge-bypass correction

R140 แก้ปัญหาที่เกิดเฉพาะ GitHub: ปุ่มเชื่อมต่อใหม่และ recovery probe ยังเรียก `ensureBridgeClient()`/`ping()` แบบ bridge ทั้งที่ `persistentGasIframeBridgeDisabled=true` ทำให้ผู้ใช้เห็นข้อความ `GAS bridge ต้องพร้อมใช้งานใน GitHub mode: ใช้ authenticated POST แทน` แม้ GAS direct จะเข้าได้ปกติ

การแก้ไข:

- `AppTransport.ensureBridgeClient()` ใน GitHub mode คืนสถานะ `Bridge-primary / POST fallback ready` แทนการ throw error
...
- Diagnostic page เปลี่ยนเป็น Bridge-primary / POST fallback diagnostic
