(function(w,d){
  ...
  var OWNER="github-pages/github-gas-transport.js::canonical-rpc-resilient";
  ...
  var metrics={calls:0,healthChecks:0,healthRetries:0,posts:0,polls:0,pendingPolls:0,resultRetries:0,completed:0,failed:0,dedupeHits:0};
  ...
  function jsonp(mode,fields,timeoutMs){return new Promise(function(resolve,reject){
    ...
    tm=w.setTimeout(function(){var ms=Math.max(2500,Number(timeoutMs||5000)),label=mode==="github-rpc-health"?"health":"result";finish(false,{code:mode==="github-rpc-health"?"GAS_RPC_HEALTH_TIMEOUT":"GAS_RPC_RESULT_TIMEOUT",message:"GAS RPC "+label+" timeout หลัง "+Math.round(ms/1000)+" วินาที"})},Math.max(2500,Number(timeoutMs||5000)));
    sc.src=build(mode,fields);(d.head||d.documentElement).appendChild(sc);
  })}

  function transientJsonp(e){var c=text(e&&(e.code||e.errorCode));return c==="GAS_RPC_HEALTH_TIMEOUT"||c==="GAS_RPC_RESULT_TIMEOUT"||c==="GAS_RPC_JSONP_TIMEOUT"||c==="GAS_RPC_JSONP_LOAD_FAILED"}
  function healthCheck(force){
    ...
    var maxRetries=Math.max(0,Math.min(2,Number(cfg("RPC_HEALTH_RETRIES",1))||0)),timeoutMs=Math.max(5000,Number(cfg("RPC_HEALTH_TIMEOUT_MS",20000))||20000);
    function attempt(n){return jsonp("github-rpc-health",{parentOrigin:w.location.origin,rpcVersion:RPC_VERSION},timeoutMs).catch(function(e){if(n<maxRetries&&transientJsonp(e)){metrics.healthRetries++;return new Promise(function(r){w.setTimeout(r,750)}).then(function(){return attempt(n+1)})}throw e})}
    healthPromise=attempt(0).then(function(x){
  ...
  function poll(rec,n){n=Number(n||0)||0;if(Date.now()>=rec.deadline)return Promise.reject(err({code:"GAS_RPC_REQUEST_TIMEOUT",message:"GAS RPC request timeout: "+rec.original}));metrics.polls++;var remaining=rec.deadline-Date.now(),pollTimeout=Math.min(Math.max(5000,Number(cfg("RPC_RESULT_JSONP_TIMEOUT_MS",20000))||20000),Math.max(5000,remaining));return jsonp("github-rpc-result",{parentOrigin:w.location.origin,rpcVersion:RPC_VERSION,rpcId:rec.id,rpcToken:rec.token},pollTimeout).then(function(x){if(x&&x.pending===true){metrics.pendingPolls++;return new Promise(function(r){w.setTimeout(r,pollDelay(n+1))}).then(function(){return poll(rec,n+1)})}if(!x||x.transportOk===false||x.ok===false)throw err(x&&x.error||{code:"GAS_RPC_RESULT_FAILED",message:"GAS RPC result failed"});return x.result}).catch(function(e){if(transientJsonp(e)&&Date.now()<rec.deadline-1000){metrics.resultRetries++;return new Promise(function(r){w.setTimeout(r,pollDelay(n+1))}).then(function(){return poll(rec,n+1)})}throw e})}
