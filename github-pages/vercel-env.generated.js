/* Production compatibility bootstrap: contracts only. Page UI owners live in deferred page runtimes. */
(function(root,doc){
  "use strict";
  if(!root) return;
  root.__VERCEL_MIGRATION_CONFIG__=root.__VERCEL_MIGRATION_CONFIG__||{};
  var OWNER="github-pages/runtime-bootstrap-production-current";
  ...
  var permissionRaw={stamp:"permission-matrix-production-current",owner:"Code_02_Platform_Permissions.APP_PERMISSION_MATRIX",defaultRole:"Viewer",roles:roles,pageAliases:aliases,apiGroupMinimumRole:apiGroups,deferredAssetMinimumRole:deferredRoles,bundleMinimumRole:bundleRoles};
  ...
  /* SafeDOM bootstrap required before Core Runtime arrives. */
  ...
  /* Thai date contract required by Search/Report before the full core is loaded. */
  ...
  Format.__thaiDateContractOwner="Index:AppFormat.canonical-thai-date-current";

  /* Auth facade required directly by Meeting/Budget runtimes. */
  ...
  /* One canonical deferred-asset map. */
  ...
  root.__APP_ASSET_MANIFEST__={stamp:"asset-manifest-production-current",sourceOwner:"gas-backend/Code_03_Platform_Assets",bundles:bundles,chunks:chunks,templates:{},upfrontScripts:[],externalGroups:["bootstrap","xlsx"]};
  ...
  /* Retire the old generated agenda owner without changing the r271/r272 router file. */
  root.__COMMITTEE_MEETING_AGENDA_UPFRONT_OWNER_R55__=root.__COMMITTEE_MEETING_AGENDA_UPFRONT_OWNER_R55__||"retired-by-production-current-single-owner";

  if(doc){
    try{
      doc.documentElement.setAttribute("data-runtime-bootstrap","production-current");
      doc.documentElement.setAttribute("data-safe-html-bootstrap","production-current");
      doc.documentElement.setAttribute("data-thai-date-contract","production-current");
      doc.documentElement.setAttribute("data-auth-facade","production-current");
      doc.documentElement.setAttribute("data-dashboard-ui-owner","Scripts_Page_Dashboard");
      doc.documentElement.setAttribute("data-committee-agenda-owner","Scripts_Page_Meeting");
      doc.documentElement.setAttribute("data-permission-matrix-stamp",permissionRaw.stamp);
    }catch(_e){}
  }
  root.__APP_RUNTIME_DEPENDENCY_MAP__={ok:true,owner:OWNER,stamp:"production-current",core:root.__APP_CORE_RUNTIME_FILES__.slice(),pages:Object.keys(chunks),authFacade:true,externalAssets:true,pageUiOwners:{dashboard:"Scripts_Page_Dashboard",committeeAgenda:"Scripts_Page_Meeting"}};
