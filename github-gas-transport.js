...
*   - Login: login POST only; fast-login JSONP is disabled in Phase G.
...
var PHASE2_RELEASE_STAMP = 'commission-v1.1-phaseK-write-schema-unification-2026-07-02-r1';
...
if (!list.length) list = ['apiGetRouteContract','apiGetPhase0ContractGate','apiGetPhase1Contract','apiGetPhase2Contract'];
...
method = text(method).trim();
/* Phase C transport contract:
- Public read API: JSONP only for explicitly allow-listed public methods.
- Authenticated read API: hidden GAS bridge iframe only; never send token/csrf via JSONP URL.
- Write/mutation API: hidden GAS bridge iframe only (handled by AppTransport.run fallback). */
var isPublicJsonp = publicJsonpReadMethod(method);
var transport = isPublicJsonp ? 'jsonp' : 'bridge';
var runner = function() {
if (isPublicJsonp) return runJsonpApi(method, payload || {});
...
function bridgeError(message, code, method) { var err = new Error(message); err.code = code || 'GAS_TRANSPORT_ERROR'; err.errorCode = err.code; err.method = method || ''; err.transportMode = cfg('transportMode', 'phaseK-write-schema-unification-auth-bridge-ready-login-post-only'); return err; }
...
var iframe = doc.createElement('iframe'); var readyTimer = null; var probeTimer = null; var settled = false;
...
(data.type === 'GAS_IFRAME_TRANSPORT_READY' || data.type === 'GAS_BRIDGE_READY') &&
data.ready === true;
}
...
bridgeClient.messageOrigin = text(event && event.origin || bridgeClient.messageOrigin || '');
...
iframe.addEventListener('load', function() { bridgeClient.loaded = true; sendReadyProbe(iframe); });
iframe.src = bridgeClientSrc(url);
bridgeClient.iframe = iframe;
root.addEventListener('message', onReady);
probeTimer = setInterval(function() { sendReadyProbe(iframe); }, 350);
readyTimer = setTimeout(function() {
bridgeClient.ready = false;
bridgeClient.assumedReady = false;
bridgeClient.promise = null;
finish(false, bridgeError('Phase G: GAS bridge ยังไม่พร้อม — ต้องได้รับ GAS_IFRAME_TRANSPORT_READY จาก iframe จริง ไม่ถือว่า iframe load แล้วพร้อมใช้งาน', 'GAS_BRIDGE_READY_MESSAGE_TIMEOUT'));
}, Number(cfg('bridgeReadyTimeoutMs', 10000)) || 10000);
...
return Promise.reject(bridgeError('Phase G: fast-login JSONP ถูกปิดถาวร เพราะเสี่ยงออก session จาก username โดยไม่ใช้ password ให้ใช้ login POST iframe เท่านั้น', 'FAST_LOGIN_JSONP_DISABLED', 'apiLogin'));
}
...
if (/^(password|pass|pwd|token|_token|authToken|csrfToken|csrf|_csrf|actionToken|csrfActionToken|_actionToken)$/i.test(k)) return;
...
out.githubPublicReadOnly = true;
out.source = out.source || 'github-public-jsonp-read-phaseC';
return out;
}
...
var url = resolveGasUrl();
method = text(method).trim();
if (!publicJsonpReadMethod(method)) return Promise.reject(bridgeError('Phase C อนุญาต JSONP เฉพาะ public read เท่านั้น: ' + method, 'JSONP_PUBLIC_READ_ONLY', method));
...
var configStamp = text(cfg('releaseStamp', PHASE2_RELEASE_STAMP) || PHASE2_RELEASE_STAMP);
var manifestObj = (root.APP_CONFIG && root.APP_CONFIG.assetManifest) || root.__APP_ASSET_MANIFEST__ || {};
var assetStamp = text((manifestObj && manifestObj.stamp) || cfg('assetStamp', ''));
var publicStamp = text(lastPublicConfig && (lastPublicConfig.releaseStamp || lastPublicConfig.stamp) || '');
var bridgeStamp = text(lastPublicConfig && (lastPublicConfig.bridgeStamp || lastPublicConfig.stamp) || '');
var backendAssetStamp = text(lastPublicConfig && lastPublicConfig.assetStamp || '');
var mismatch = [];
if (configStamp && configStamp !== PHASE2_RELEASE_STAMP) mismatch.push({ field:'client-config-releaseStamp', expected:PHASE2_RELEASE_STAMP, actual:configStamp });
if (publicStamp && publicStamp !== PHASE2_RELEASE_STAMP) mismatch.push({ field:'backend-public-releaseStamp', expected:PHASE2_RELEASE_STAMP, actual:publicStamp });
if (bridgeStamp && bridgeStamp !== PHASE2_RELEASE_STAMP) mismatch.push({ field:'backend-bridge-stamp', expected:PHASE2_RELEASE_STAMP, actual:bridgeStamp });
if (backendAssetStamp && assetStamp && backendAssetStamp !== assetStamp) mismatch.push({ field:'assetStamp', expected:assetStamp, actual:backendAssetStamp });
lastReleaseMismatch = mismatch.length ? mismatch : null;
return { ok: !mismatch.length, expectedStamp: PHASE2_RELEASE_STAMP, clientStamp: PHASE2_RELEASE_STAMP, configStamp: configStamp, assetStamp: assetStamp, backendReleaseStamp: publicStamp, backendBridgeStamp: bridgeStamp, backendAssetStamp: backendAssetStamp, mismatch: mismatch };
}
...
root[cb] = function(data) { if (done) return; done = true; clearTimeout(timer); try { script.parentNode && script.parentNode.removeChild(script); } catch (_) {} try { delete root[cb]; } catch (_) { root[cb] = void 0; } if (data && data.ok) { lastPublicConfig = data; var logo = text(data.logoUrl || (data.appLogo && (data.appLogo.active || data.appLogo.svg)) || ''); logo && !isBadLogo(logo) && setLogo(logo, 'gas-public-config'); try { var rel = releaseStatus(); if (!rel.ok) recordApiMetric({ kind:'release-mismatch', method:'__githubPublicConfig', transport:'jsonp', error:true, message:JSON.stringify(rel.mismatch) }); } catch (_) {} } resolve(data || null); };
...
root.AppTransport.transportMode = cfg('transportMode', 'phaseK-write-schema-unification-auth-bridge-ready-login-post-only');
root.AppTransport.bridgeClientState = function() { return { ready: !!bridgeClient.ready, loaded: !!bridgeClient.loaded, assumedReady: !!bridgeClient.assumedReady, messageOrigin: bridgeClient.messageOrigin || '', url: bridgeClient.url || resolveGasUrl(), mode: cfg('transportMode', 'phaseK-write-schema-unification-auth-bridge-ready-login-post-only') }; };
root.AppTransport.phase2Status = function() { var release = releaseStatus(); return { ok: release.ok, stamp: PHASE2_RELEASE_STAMP, phase: 'Phase K Write Schema Unification', release: release, releaseMismatch: release.mismatch, authenticatedReadBridge: true, readJsonpFallbackToBridge: false, firstPaint: cfg('dashboardFirstPaintEnabled', true) !== false, lazyHydration: cfg('dashboardLazyHydrationEnabled', true) !== false, budgetHydration: cfg('dashboardBudgetHydrationEnabled', true) !== false, dashboardInitialIncludeBudget: cfg('dashboardInitialIncludeBudget', false) === true, phaseIContractFinalCleanup: cfg('phaseIContractGateEnabled', true) !== false, phaseJRuntimeSlimming: cfg('phaseJRuntimeSlimming', true) !== false, phaseKWriteSchemaUnification: cfg('phaseKWriteSchemaUnification', true) !== false, phaseGSecurityHardening: cfg('phaseGSecurityHardening', true) !== false, fastLoginJsonpDisabled: cfg('fastLoginJsonp', false) === false, requireBridgeReadyMessage: cfg('requireBridgeReadyMessage', true) !== false, allowAssumedBridgeReady: cfg('allowAssumedBridgeReady', false) === true, releaseGate: cfg('phaseIContractGate', 'tools/phaseK_write_schema_gate.py'), syncTool: cfg('phaseISyncTool', 'tools/sync_frontend_partials.py'), singleSourceRoot: cfg('phase2CanonicalPartialRoot', 'gas-backend'), generatedMirrorPolicy: cfg('phase2GeneratedMirrorPolicy', 'edit-gas-backend-run-sync-do-not-edit-generated-mirrors'), clientApiCacheEnabled: cfg('clientApiCacheEnabled', true) !== false, clientInFlightDedupe: cfg('clientInFlightDedupe', true) !== false, jsonpCircuitOpen: (Date.now ? Date.now() : +new Date()) < jsonpCircuitUntil, jsonpCircuitReason: jsonpCircuitReason, strictBridgeOriginCheck: cfg('strictBridgeOriginCheck', true) !== false, publicJsonpReadMethods: cfgList('publicJsonpReadMethods'), cacheEntries: Object.keys(apiCache).length, inFlight: Object.keys(apiInFlight).length, metrics: Object.assign({}, apiMetrics), bridge: root.AppTransport.bridgeClientState() }; };
...
if (cfg('fastLoginJsonp', false) === false) throw postErr;
